/* V10：状态连续台词模式 / 平滑逐字头像心声框 / 原生 dialog 设置界面 / 可调颜色 / 任意心情头像
...
5. 设置界面支持心情头像、对话框颜色、头像框颜色、边框颜色、文字颜色。V10 已新增状态连续台词模式，并保留旧版渐变纹理与平滑逐字。
*/

(() => {
  const SCRIPT_FLAG = '__ST_HEART_HB_NATIVE_DIALOG_SEQUENCE_V10__';
  ...
  const BUTTON_BG = '底图·HB10';
  const BUTTON_SETTINGS = '设置·HB10';
  ...
  function getTheme() {
    ...
    try {
      ...
      console.warn('[头像心声框 V10] 读取颜色配置失败，使用默认颜色：', e);
    }
  ...
  function saveTheme(theme) {
    ...
    try {
      ...
      console.warn('[头像心声框 V10] 保存颜色配置失败：', e);
    }
  ...
  function getAvatarPools() {
    ...
    try {
      ...
      console.warn('[头像心声框 V10] 读取头像配置失败，使用默认头像：', e);
    }
  ...
  function saveAvatarPools(pools) {
    ...
    try {
      ...
      console.warn('[头像心声框 V10] 保存头像配置失败：', e);
    }
  }
  ...
  function injectStyleInDoc(doc) {
    ...
    if (doc.getElementById('st-heart-hb-style-v10')) return;

    const style = doc.createElement('style');
    style.id = 'st-heart-hb-style-v10';
    style.textContent = `
      .st-heart-hb-box,
      .st-heart-hb-box * {
  ...
  function makeHBHash(data) {
    if (data?.mode === 'sequence') {
      return hashString([
        data.id,
        'sequence',
        ...(Array.isArray(data.segments) ? data.segments.map((item) => `${item.mood}：${item.text}`) : []),
      ].join('|'));
    }
  ...
  function parseSequenceLines(block, messageId) {
    ...
    lines.forEach((line) => {
      ...
      if (match) {
        const mood = cleanMoodName(match[1]);
        const text = String(match[2] || '').trim();
        const lowerMood = mood.toLowerCase();

        // outer / inner 属于旧格式，不当成连续台词状态。
        if ((lowerMood === 'outer' || lowerMood === 'inner') && !text) return;

        if (text) {
          segments.push({ mood, text });
        }
  ...
  function buildHeartBox(data, doc) {
    ...
    if (data?.mode === 'sequence' && Array.isArray(data.segments) && data.segments.length) {
      const segments = data.segments.map((segment) => ({
        mood: cleanMoodName(segment.mood),
        text: String(segment.text || '').trim(),
        avatar: getAvatarByMood(segment.mood),
      ...
      function renderSequence() {
        ...
        img.alt = current.mood || '普通';
        box.dataset.currentMood = current.mood || '普通';
    ...
    function renderToggle() {
      ...
      img.alt = currentMood;
      box.dataset.currentMood = currentMood || '普通';
  ...
  function openSettingsDialog() {
    ...
    try {
      ...
      console.error('[头像心声框 V10] 打开设置 dialog 失败：', e);
  ...
  function setupButtons() {
    ...
    if (typeof replaceButtons !== 'function' || typeof onButton !== 'function') {
      console.warn('[头像心声框 V10] 未找到酒馆助手按钮 API：replaceScriptButtons / eventOnButton');
      return;
    }
  ...
  function safeEventOn(eventName, handler) {
    try {
      ...
      console.warn('[头像心声框 V10] 事件绑定失败：', eventName, e);
    }
  }
  ...
  function boot() {
    ...
    console.log('[头像心声框 V10] loaded：状态连续台词模式 / 原生 dialog 设置界面 / 可调颜色 / 任意心情头像');
  }
