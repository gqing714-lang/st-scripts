/* V8：恢复旧版质感的头像心声框 / 原生 dialog 设置界面 / 可调颜色 / 任意心情头像
...
5. 设置界面支持心情头像、对话框颜色、头像框颜色、边框颜色、文字颜色。V8 已恢复旧版渐变纹理与轻量逐字。
*/

(() => {
  const SCRIPT_FLAG = '__ST_HEART_HB_NATIVE_DIALOG_TEXTURE_V8__';
  ...
  const BUTTON_BG = '底图·HB8';
  const BUTTON_SETTINGS = '设置·HB8';
  ...
  function getTheme() {
    ...
    try {
      ...
      console.warn('[头像心声框 V8] 读取颜色配置失败，使用默认颜色：', e);
    }
  ...
  function saveTheme(theme) {
    ...
    try {
      ...
      console.warn('[头像心声框 V8] 保存颜色配置失败：', e);
    }
  ...
  function getAvatarPools() {
    ...
    try {
      ...
      console.warn('[头像心声框 V8] 读取头像配置失败，使用默认头像：', e);
    }
  ...
  function saveAvatarPools(pools) {
    ...
    try {
      ...
      console.warn('[头像心声框 V8] 保存头像配置失败：', e);
    }
  }
  ...
  function injectStyleInDoc(doc) {
    ...
    if (doc.getElementById('st-heart-hb-style-v8')) return;

    const style = doc.createElement('style');
    style.id = 'st-heart-hb-style-v8';
    style.textContent = `
      .st-heart-hb-box,
      .st-heart-hb-box * {
  ...
  function openSettingsDialog() {
    ...
    try {
      ...
      console.error('[头像心声框 V8] 打开设置 dialog 失败：', e);
  ...
  function setupButtons() {
    ...
    if (typeof replaceButtons !== 'function' || typeof onButton !== 'function') {
      console.warn('[头像心声框 V8] 未找到酒馆助手按钮 API：replaceScriptButtons / eventOnButton');
      return;
    }
  ...
  function safeEventOn(eventName, handler) {
    try {
      ...
      console.warn('[头像心声框 V8] 事件绑定失败：', eventName, e);
    }
  }
  ...
  function boot() {
    ...
    console.log('[头像心声框 V8] loaded：原生 dialog 设置界面 / 可调颜色 / 任意心情头像');
  }
