/* 正式增强版：状态连续台词模式 / 平滑逐字头像心声框 / 原生 dialog 设置界面 / 可调颜色和字体 / 任意状态头像池 / 本地图片上传 / 角色卡绑定
...
7. 支持将配置写入当前角色卡 extensions 字段，便于随角色卡导出。
*/

(() => {
  const SCRIPT_FLAG = '__ST_HEART_HB_DIALOG_RELEASE_CARD_UPLOAD__';

  if (window[SCRIPT_FLAG]) return;
  window[SCRIPT_FLAG] = true;

  const VERSION = 'RELEASE_CARD_UPLOAD';
  const SPEED_MS = 42;
  ...
  function saveStatusPools(pools) {
    ...
    try {
      ...
      try {
        getP().toastr?.warning?.('头像配置保存到本地失败：图片可能太多或太大，但仍可尝试保存到角色卡。');
      } catch (_) {}
    }
  }
  ...
  async function saveConfigToCurrentCharacter(config) {
    ...
    await ctx.writeExtensionField(characterId, CARD_EXTENSION_KEY, buildPortableConfig(config));
  ...
  function injectStyleInDoc(doc) {
    ...
    if (doc.getElementById('st-heart-hb-style-release-card')) return;

    const style = doc.createElement('style');
    style.id = 'st-heart-hb-style-release-card';
    style.textContent = `
      .st-heart-hb-box,
      .st-heart-hb-box * {
  ...
  function setPanelFromConfig(container, config) {
    ...
    Object.keys(clean.theme).forEach((key) => {
        ...
        <div class="st-heart-hb-settings-grid-v11">
          ...
          <section class="st-heart-hb-section-v11">
            ...
            <div class="st-heart-hb-card-panel-v11">
              ...
              <div class="st-heart-hb-tip-v11">
                点击“保存到当前角色卡”后，状态图包网址、本地上传图片、颜色、字体和头像底图开关会写入当前角色卡。导出角色卡时会跟着走；其他人仍需要加载本脚本才能渲染。<br><br>
                本地图片会变成图片数据写入角色卡，头像越多，角色卡文件越大。
              </div>
              <div class="st-heart-hb-actions-v11">
  ...
  function boot() {
    ...
    console.log('[头像心声框 正式增强版] loaded：状态连续台词模式 / 本地图片上传 / 角色卡绑定 / 可调颜色和字体 / 任意状态头像池');
  }
