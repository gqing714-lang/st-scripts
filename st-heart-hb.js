/* V4：原生弹窗设置稳定版。点「头像设置」使用 prompt/alert，不再生成 HTML 设置面板，避免手机端被压成褐色长条。 */
/* 头像心声框：可视化头像配置版 / 任意心情名 / 轻量逐字 / 紫红底色 / 原生弹窗设置稳定版 / 保留最新5个
...
(() => {
  if (window.__ST_HEART_HB_PROMPT_SETTINGS_V4__) return;
  window.__ST_HEART_HB_PROMPT_SETTINGS_V4__ = true;
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      #st-heart-hb-avatar-config-overlay.st-heart-hb-inline-settings {
      ...
      #st-heart-hb-avatar-config-overlay.st-heart-hb-inline-settings .st-heart-hb-config-panel {
        width: 100% !important;
        max-height: min(720px, 74vh) !important;
        margin: 0 auto !important;
      }

      #st-heart-hb-avatar-config-overlay.st-heart-hb-inline-settings .st-heart-hb-config-body {
  ...
  function getSettingsPanelHost(doc) {
    // 手机端酒馆助手有时会把 fixed 弹窗限制在按钮栏的小 iframe 里，
    // 所以设置面板优先插入到聊天消息区域，而不是做全屏浮层。
    try {
      const lastData = findLastHB();
      const lastHost = lastData ? getMessageHost(lastData.id) : null;
  ...
  function openAvatarSettingsPanel() {
    const pools = clonePools(getAvatarPools());

    const currentMoods = Object.keys(pools).join(' / ') || '暂无';
    ...
    if (choice === '1') {
      ...
      const oldUrls = Array.isArray(pools[cleanMood]) ? pools[cleanMood].join('\n') : '';
      ...
      pools[cleanMood] = Array.from(new Set(urls));
      saveAvatarPools(pools);
      rebuildVisibleHeartBoxesAfterConfigChange();

      alert('已保存心情「' + cleanMood + '」的头像图包，共 ' + pools[cleanMood].length + ' 张。');
      return;
    }
    ...
    if (choice === '2') {
      ...
      if (!pools[cleanMood]) {
        alert('没有找到心情「' + cleanMood + '」。');
        return;
      }

      delete pools[cleanMood];

      if (!Object.keys(sanitizePools(pools)).length) {
        alert('不能删到一个心情都不剩。');
        return;
      }
      ...
      alert('已删除心情「' + cleanMood + '」。');
      return;
    }

    if (choice === '3') {
      const ok = confirm('确定恢复默认头像配置吗？这会清掉你自定义的心情图包。');
      ...
      try {
        getStorage().removeItem(STORAGE_KEY_AVATAR_POOLS);
      } catch (e) {}
    ...
    if (choice === '4') {
      ...
      const text = Object.keys(clean)
        .map((mood) => {
          return mood + '：\n' + clean[mood].map((url, index) => '  ' + (index + 1) + '. ' + url).join('\n');
        })
        .join('\n\n');
  ...
  console.log('[头像心声框：可视化头像配置版 / 任意心情 / 原生弹窗设置稳定版 / 保留最新5个] loaded');
})();
